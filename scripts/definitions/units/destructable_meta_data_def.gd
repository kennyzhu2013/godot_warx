class_name DestructableMetaDataDef
extends Resource

## Units/DestructableMetaData.slk 一行定义。
##
## 职责：对象编辑器「可破坏物」字段元数据，非可破坏物实例数据。

const TABLE_NAME := "DestructableMetaData"
const SLK_REL_PATH := "Units/DestructableMetaData.json"
...
static func from_slk_record(rec: Dictionary) -> DestructableMetaDataDef:
	var d := DestructableMetaDataDef.new()
...
static func register_to(store: Node) -> void:
	if store == null or not store.has_method("register_table"):
		push_error("DestructableMetaDataDef: 无法注册到 DefStore")
		return
