...
## 单位增删改命令。笔刷已写入 Document 时用 history.record()；undo/redo 走 execute/undo。
...
static func make_add(entries: Array, label: String = "Place Unit") -> EditorCommand:
	var cmd = new()
	cmd._op = Op.ADD
	cmd._label = label
	for e in entries:
...
static func make_remove(entries: Array, label: String = "Delete Unit") -> EditorCommand:
	var cmd = new()
	cmd._op = Op.REMOVE
	cmd._label = label
	for e in entries:
...
static func make_modify(befores: Array, afters: Array, label: String = "Edit Unit") -> EditorCommand:
	var cmd = new()
	cmd._op = Op.MODIFY
	cmd._label = label
